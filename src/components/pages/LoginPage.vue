...
async function onSubmit() {
  form.loading = true
  try {
    /* Gọi API đăng nhập qua store */
    await auth.login(form.username, form.password)

    message.success('Đăng nhập thành công')
    router.replace((route.query.redirect as string) || '/')
  } catch (err: any) {
    message.error(err.response?.data.message || 'Email hoặc mật khẩu sai')
  } finally {
    form.loading = false
  }
}
...
.login-wrapper {
  ...
  align-items: center;
  background: url('@/assets/background3.svg') center/cover no-repeat;
}
.login-wrapper .ant-row {
  width: 100%;
  border-radius: 12px;
...
.logo {
  ...
  z-index: 10;
  pointer-events: none;
  animation: floatIn 1s ease-out 1 forwards;
  opacity: 0.8;
...
.decor-blue-circle {
  ...
  pointer-events: none;
  z-index: 1;
  animation: floatIn 0.8s ease-out 1 forwards;
}
@keyframes floatIn {
  0% {
...
.decor-doctor {
  ...
  pointer-events: none;
  z-index: 2;
  animation: riseUp 0.8s ease-out 1 forwards;
}
...
.header {
  ...
  left: 0;
  width: 100vw;
  height: 150px; /* chiều cao header AntD mặc định */
  /* padding: 0 24px; */
  display: flex;
  flex-direction: row;
  z-index: 1000;
...
.app-header1 {
  ...
  width: 50%;
  right: 20px;
  height: 64px; /* chiều cao header AntD mặc định */
  z-index: 1000;
  background: none;
}
.app-header {
...
.ant-card {
  ...
  backdrop-filter: blur(10px); /* hiệu ứng mờ nền (glassmorphism) */
